...
QTWebStompClient::QTWebStompClient(const char* url, const char* login, const char* passcode, const char* vHost, bool debug, QObject *parent)
{
    m_url = QUrl(url);
    m_debug = debug;
    m_login = login;
...
void QTWebStompClient::onTextMessageReceived(QString message)
{
    ...
    case Connecting:
        ...
        else
        {
            ...
            emit submitError(QString("Message type CONNECTED expected, got %1").arg(QString::fromStdString(stompMessage.toString())));
        }
        break;
    ...
    case Subscribed:
        ...
        if (stompMessage.m_messageType == "MESSAGE") {
            ...
            emit submitError(QString("Message type MESSAGE expected, got %1. Message is : %2").arg(QString::fromStdString(stompMessage.m_messageType), QString::fromStdString(stompMessage.toString())));
        }
        break;

    default:
        emit submitError("Unsupported connection state");
        break;
    }
...
{
    emit submitError("SSL error! I'd show the error description if there were an easy way to convert from enum to string in c++. You'll have to debug.");
}
